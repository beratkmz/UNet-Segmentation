...
def pred_show_image_grid(data_path, model_pth, device):
    ...
    for img, orig_mask in image_dataset:
        ...
        orig_mask = orig_mask.squeeze(0).cpu().detach()
    ...
    for i in range(len(images)):
        fig.add_subplot(3, len(image_dataset), i+1)
...
def single_image_inference(image_pth, model_pth, device):
    ...
    img = transforms(Image.open(image_pth)).float().to(device)
    img = img.unsqueeze(0)
...
if __name__ == "__main__":
    SINGLE_IMG_PATH = "./src/data/manual_test/test2.jpg"
    DATA_PATH = "./src/data"
    MODEL_PATH = "./src/models/unet.pth"
